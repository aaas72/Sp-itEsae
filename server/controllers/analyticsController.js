  ...
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(groupId)) {
      ...
      return res.status(400).json(
        createErrorResponse('INVALID_ID', 'Geçersiz grup kimliği biçimi')
      );
    }
    ...
    if (!group || !group.members.some(m => m.userId.toString() === userId)) {
      ...
      return res.status(404).json(
        createErrorResponse('GROUP_NOT_FOUND', 'Grup bulunamadı veya bu grubun üyesi değilsiniz')
      );
    }
    ...
    res.status(200).json(createSuccessResponse(stats, 'Grup toplam tutarları başarıyla getirildi'));
  } catch (error) {
...
const getMonthlyAnalysis = async (req, res) => {
  try {
    ...
    res.status(200).json(
      createSuccessResponse(simplifiedMonthlyData, 'Aylık analiz başarıyla getirildi')
    );
  } catch (error) {
...
const getUserPatterns = async (req, res) => {
  try {
    ...
    res.status(200).json(
      createSuccessResponse(userPatterns, 'Kullanıcı harcama kalıpları başarıyla getirildi')
    );
  } catch (error) {
...
const getGroupSummary = async (req, res) => {
  try {
    ...
    res.status(200).json(
      createSuccessResponse(stats, 'Grup özet istatistikleri başarıyla getirildi')
    );
  } catch (error) {
