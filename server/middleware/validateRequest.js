    ...
    try {
        ...
        if (!Array.isArray(rules)) {
            ...
            return res.status(500).json(
                createErrorResponse('INTERNAL_ERROR', 'Doğrulama kurallarında sunucu yapılandırma hatası.')
            );
        }
        ...
        rules.forEach(rule => {
            ...
            if (rule.required && (value === undefined || value === null || String(value).trim() === '')) {
                errors.push(`${rule.field} gereklidir.`);
            }
        });
        ...
        return res.status(500).json(
            createErrorResponse('INTERNAL_ERROR', 'İstek doğrulaması sırasında beklenmeyen bir hata oluştu.')
        );
    }
  };
};
